...
function normalizeName(s) {
  return String(s || "")
    .replace(/[ÆæÄä]/g, "ae").replace(/[ØøÖö]/g, "oe").replace(/[Åå]/g, "aa")
...
function search(query, stores, limit = 40) {
  ...
  if (q.length < 3 || !CAT.ready) return [];
...
function simulate(g, startTs, days) {
  ...
  return { rows: out, mu, firstEmpty, totalWaste };
}
...
window.Kurv = {
  ...
  simulate, spendSeries, classSummary,
  money, num, toast, todayISO, isoToTs, tsToISO, dayFloor, fmtDate, daysBetween, normalizeName,
};
